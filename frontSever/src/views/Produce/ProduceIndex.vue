...
export default {
  setup() {
    ...
    const fetchData = (val) => {
      ...
      axios
        .get(`http://121.37.243.173:8081/nameSelectProduce?name=${val}`, {
          headers,
        })
        .then((response) => {
    ...
    const deleteProduce = (ProduceId, position) => {
      axios
        .delete(`http://121.37.243.173:8081/deleteProduce?id=${ProduceId}`, {
          headers,
        })
        .then((response) => {
          // 处理删除成功的逻辑
    ...
    const buyItem = (index, position, item) => {
      axios
        .get(`http://121.37.243.173:8081/selectCredit?name=${item.customer}`, {
          headers,
        })
        .then((response) => {
      ...
      axios
        ...
        .then((response) => {
          ...
          if (buyCredit >= item.produceCredit) {
            ...
            axios
              .put(
                `http://121.37.243.173:8081/buyProduce?id=${ProduceId}`,
                null,
                {
                  headers,
                }
    ...
    const loveItem = (item) => {
      axios
        .get(
          `http://121.37.243.173:8081/updateLoveProduce?id=${item.produceId}`,
          {
            headers,
          }
        )
