    ...
    <div class="content_home">
        <van-notice-bar color="#333333" background="#FFB6C1" left-icon="volume-o" :scrollable="false">
            <van-swipe vertical class="notice-swipe" :autoplay="3000" :touchable="true" :show-indicators="false">
                <van-swipe-item>✨欢迎来到捏捏的魔仙堡🏰</van-swipe-item>
                <van-swipe-item>👑捏捏最可爱</van-swipe-item>
                <van-swipe-item>小羊最帅气</van-swipe-item>
...
export default {
    setup() {
        ...
        const fetchData = () => {
            ...
            axios.get('http://40.121.58.85:8081/user', { headers })
                .then(response => {
        ...
        const requireItem = (item) => {
            ...
            if (userId == item.nameId) {
                const taskIdToDelete = item.taskId;
                isLoading.value = true;
                // 调用 deleteTask 函数来发送 HTTP DELETE 请求
                requireTask(taskIdToDelete);
            }
            else show_1.value = true;
        };
        const requireTask = (taskId) => {
            router.push(`/UserRequire/${taskId}`);
            isLoading.value = false;
