<template>
    <van-nav-bar title="用户" left-text="" left-arrow @click-left="onClickLeft" />
...
export default {
    setup() {
        ...
        const fetchDataAndFillForm = async () => {
            try {
                const response = await axios.get(`http://40.121.58.85:8081/selectUser?id=${strippedId}`, { headers });
