...
export default {
    ...
    setup() {
        ...
        const fetchData_0 = (val) => {
            ...
            axios.get(`http://40.121.58.85:8081/nameNoSelectRoom?name=${val}`, { headers })
                .then(response => {
        ...
        const fetchData_1 = (val) => {
            ...
            axios.get(`http://40.121.58.85:8081/nameYesSelectRoom?name=${val}`, { headers })
                .then(response => {
        ...
        const deleteProduce = (produceId, position, checkProduce_tem) => {
            ...
            axios.delete(`http://40.121.58.85:8081/deleteRoom?id=${produceId}`, { headers })
                .then(response => {
                    // 处理删除成功的逻辑
        ...
        const checkProduce = async (produceId, position, item) => {
            try {
                const response = await axios.get(`http://40.121.58.85:8081/idSelectRoom?id=${item.produceId}`, { headers });
                ...
                if (checkCredit > 0) {
                    ...
                    const res = await axios.put(`http://40.121.58.85:8081/reduceRoom?id=${item.produceId}`, null, { headers });
                    if (res.data.code == 1) {
                    ...
                    const postResponse = await fetch('http://40.121.58.85:8081/addRoom', {
                        method: 'POST',
                        headers: {
                    ...
                    console.log('任务已保存成功');
                    const selectResponse = await axios.get(`http://40.121.58.85:8081/selectId`, { headers });
